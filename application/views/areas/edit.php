...
<?php
    $nombre_area = array(
        ...
        'style'             => 'width: 80%',
        'value'             => set_value('nombre_area',$area[0]->descripcion)
    );    
?>
    ...
    <div class="col-lg-pull-12" style="margin-top:10px">        
        <div class="col-md-6 col-xs-12">
            <span style="font-family:Calibri; font-weight:bold; text-transform:uppercase; color:#337AB7; margin-left:15px; padding:0; font-size:18px;">
                <i class="fa fa-edit"></i>&nbsp;Modificar &aacute;rea
            </span>
        </div>

        <div class="col-md-6 col-xs-12">
            <a href="<?php echo site_url('areas/detail/').$area[0]->id; ?>" class="btn btn-default pull-right" style="margin-right: 5px"><i class="fa fa-arrow-circle-left"></i> Regresar</a>
...
<script type="text/javascript">
    function guardar()
    {
        var id = '<?php echo $area[0]->id; ?>';
        var nombre = $("#nombre_area").val();

        if (nombre != "") 
        {
            $.ajax({
                url: '<?php echo site_url('areas/edit_action'); ?>',
                type: 'POST',
                data: {
                    id: id,
                    nombre: nombre
